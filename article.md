...
3. A git worktree is created, config files are copied, Claude launches with the ticket context already pasted.
...
9:15 AM  Ticket PROJ-42 needs a fix → run the script → Claude starts working
...
10:00 AM Review Claude's PR for PROJ-42, approve and merge
...
3. **Copies project config** — secrets, build caches, environment files. Whatever the project needs to run.
4. **Launches Claude in a new terminal** — opens your terminal of choice, starts Claude in plan mode, and pastes the ticket description into the prompt.
...
- A Go project might copy nothing at all.
...
A Jira ticket with a clear title, description, and acceptance criteria is a self-contained unit of work. It doesn't need you hovering. It needs an agent that can read the spec, understand the codebase, and produce a pull request.

Your job shifts from *doing the work* to *reviewing the work*. And reviewing a PR takes a fraction of the time it takes to write the code.
...
- **You review the output.** Claude is fast, not infallible. Treat its PRs like you'd treat any other developer's — read the diff, run the tests, push back when needed.
